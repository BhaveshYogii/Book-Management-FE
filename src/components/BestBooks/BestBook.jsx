...
const BestBook = (props) => {
    ...
    useEffect(()=>{
        ...
        try {
            axios.post('http://127.0.0.1:8000/getbooks/',{
                Key: props.keyfield,
                Order: props.order,
                Limit: props.limit,
            }).then((res)=>{
  ...
  const addToCart=(BookId,Quantity)=>{
    if(session==null){
        ...
        try {
            ...
            axios.post('http://127.0.0.1:8000/addtocart/',{
  ...
  const addToList=(BookId)=>{
    if(session==null){
        ...
        let session_key=session[1];
        try {
            axios.post('http://127.0.0.1:8000/addtolist/',{
                session_key: session_key,
                BookObj: BookId,
            }).then((res)=>{
                toast.success('Book added successfully!');
        ...
        <div>
          <div className=" grid grid-cols-1 sm:grid-cols-3 place-items-start place-justify-center gap-10 gap-y-16 lg:grid-cols-4">
            {props.BooksData.map((data) => (
              <div className="space-y-3 fav-icon-outer card p-5 flex justify-center items-center flex-col w-full" key={data.BookId}>
                <button className="fav-icon"
                onClick={(e)=>addToList(data.BookId)}>
                <IoIosHeartEmpty />
                {/* <IoMdHeart /> */}
                </button>
                <img
                ...
                <div className="flex justify-center items-center">
                  <button className="bg-gradient-to-r w-full from-primary to-secondary text-white px-4 py-2 w-[170px] rounded-sm flex items-center gap-3 hover:scale-105 duration-300"
