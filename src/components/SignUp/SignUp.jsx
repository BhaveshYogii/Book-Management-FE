...
const SignUp = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!newErrors.PhoneNo && !phoneRegex.test(formData.PhoneNo)) {
      newErrors = { ...newErrors, PhoneNo: 'Please enter a valid 10-digit phone number.' };
    ...
    if (!newErrors.Password && !PasswordRegex.test(formData.Password)) {
      newErrors = {
        ...newErrors,
        Password: 'Password must contain at least 1 capital letter, 1 special character, and 1 number. Minimum length is 8 characters.',
      };
      toast.error('Password must contain at least 1 capital letter, 1 special character, and 1 number. Minimum length is 8 characters.');
    }
  ...
  return (
    ...
    <div className="min-h-screen py-6 flex flex-col justify-center sm:py-10  bg-white dark:bg-gray-900 dark:text-white duration-200" style={{ paddingTop: '0px' }}>
		<div className="relative py-3 sm:max-w-xl sm:mx-auto" >
			<div
				className="absolute inset-0 bg-gradient-to-r from-blue-300 to-blue-600 shadow-lg transform -skew-y-6 sm:skew-y-0 sm:-rotate-6 sm:rounded-3xl sm:py-20">
			</div>
			<div className="relative px-4 py-10  bg-white shadow-lg sm:rounded-3xl sm:p-20" >
				<div className="max-w-md mx-auto w-[350px] h-[400px]">
					<div>
						<h1 className="text-2xl font-semibold dark:text-black flex justify-center items-center">Signup</h1>
					</div>
